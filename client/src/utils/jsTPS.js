...
export class EditLandmark_Transaction extends jsTPS_Transaction{
    constructor(mapID, landmarkId, prev, update, callback){
        super();
        this.mapID = mapID;
    ...
    async doTransaction(){
        const {data} = await this.updateFunction({
            variables : {_id: this.mapID, landmarkId: this.landmarkId, value: this.update}
        });
        return data;
    }
    ...
    async undoTransaction(){
        const {data} = await this.updateFunction({
            variables : {_id: this.mapID, landmarkId: this.landmarkId, value: this.prev}
        });
        if (data) console.log(data)
        return data
    }
